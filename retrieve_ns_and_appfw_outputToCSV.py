...
def main():
    try:
        ...
        csv_data = []  # List to hold rows of data for the CSV
        ...
        if namespaces:  # Check if the list is not empty
            for ns in namespaces:
                ...
                # Check if app firewalls are found
                if "items" in app_firewalls:
                    for af in app_firewalls["items"]:
                        # Only add app firewalls that are NOT in the shared namespace
                        if af["name"] not in shared_app_firewalls:
                            csv_data.append([namespace_name, af['name']])  # Add to CSV data
                else:
                    csv_data.append([namespace_name, "No app firewalls found."])  # Indicate no app firewalls

        # Step 3: Write the data to a CSV file
        with open("app_firewalls_output.csv", mode="w", newline='') as file:
            writer = csv.writer(file)
            # Write the headers
            writer.writerow(["Namespace", "App Firewall Name"])
            # Write the data rows
            writer.writerows(csv_data)
        
        print("Output has been written to 'app_firewalls_output.csv'")
